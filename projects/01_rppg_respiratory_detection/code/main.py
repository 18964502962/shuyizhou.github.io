...
def train_and_test(config, data_loader_dict):
    ...
    # === Phase 2: 呼吸检测模型 ===
    elif config.MODEL.NAME == 'RespirationNet':
        model_trainer = trainer.RespirationTrainer.RespirationTrainer(config, data_loader_dict)
    # === Phase 3: STLAM 侧向注意力呼吸检测模型 ===
    elif config.MODEL.NAME == 'STLAMNet':
        model_trainer = trainer.RespirationTrainer.RespirationTrainer(config, data_loader_dict)
    # === Phase 3: 半监督呼吸检测模型 ===
    elif config.MODEL.NAME == 'SemiSupervisedRespiration':
        model_trainer = trainer.SemiSupervisedRespirationTrainer.SemiSupervisedRespirationTrainer(config, data_loader_dict)
    # === Phase 4: 多ROI呼吸检测模型（课题核心创新）===
    elif config.MODEL.NAME == 'MultiROIRespirationNet':
        model_trainer = trainer.MultiROIRespirationTrainer.MultiROIRespirationTrainer(config, data_loader_dict)
    elif config.MODEL.NAME == 'MultiROISemiSupervised':
        model_trainer = trainer.MultiROIRespirationTrainer.MultiROISemiSupervisedTrainer(config, data_loader_dict)
    else:
...
if __name__ == "__main__":
    ...
    # Check if multi-ROI mode is enabled (for passing to loaders that support it)
    multi_roi_cfg = getattr(config, 'MULTI_ROI', None)
    use_multi_roi = multi_roi_cfg is not None and getattr(multi_roi_cfg, 'ENABLE', False)
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        ...
        #     train_loader = data_loader.SUMSLoader.SUMSLoader
            ...
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, COHFACE, \
        ...
        if (config.TRAIN.DATA.DATASET and config.TRAIN.DATA.DATA_PATH):
            # Build extra kwargs for loaders that support multi_roi_config (currently COHFACE)
            extra_kwargs = {}
            if use_multi_roi and config.TRAIN.DATA.DATASET == "COHFACE":
                extra_kwargs['multi_roi_config'] = multi_roi_cfg
            ...
            train_data_loader = train_loader(
                ...
                **extra_kwargs)
            # Multi-ROI preprocessing uses MediaPipe which is not fork-safe;
            # use num_workers=0 during preprocessing. After preprocessing
            # (DO_PREPROCESS=False), multi-workers are safe since we only load npy files.
            # Multi-ROI数据较大，Windows共享内存有限，worker数不宜过多
            num_workers_train = 0  # 单进程加载，避免F盘I/O并发问题
        ...
        # ========== 半监督学习: 加载无标签训练数据 ==========
        # 检查是否有 TRAIN_UNLABELED 配置（用于半监督学习）
        if hasattr(config, 'TRAIN_UNLABELED') and config.TRAIN_UNLABELED.DATA.DATA_PATH:
            unlabeled_dataset = config.TRAIN_UNLABELED.DATA.DATASET
            
            # 选择对应的数据加载器
            if unlabeled_dataset == "UBFC-rPPG":
                unlabeled_loader_class = data_loader.UBFCrPPGLoader.UBFCrPPGLoader
            elif unlabeled_dataset == "PURE":
                unlabeled_loader_class = data_loader.PURELoader.PURELoader
            elif unlabeled_dataset == "SCAMPS":
                unlabeled_loader_class = data_loader.SCAMPSLoader.SCAMPSLoader
            elif unlabeled_dataset == "MMPD":
                unlabeled_loader_class = data_loader.MMPDLoader.MMPDLoader
            elif unlabeled_dataset == "UBFC-PHYS":
                unlabeled_loader_class = data_loader.UBFCPHYSLoader.UBFCPHYSLoader
            elif unlabeled_dataset == "iBVP":
                unlabeled_loader_class = data_loader.iBVPLoader.iBVPLoader
            elif unlabeled_dataset == "COHFACE":
                unlabeled_loader_class = data_loader.COHFACELoader.COHFACELoader
            else:
                print(f"Warning: Unsupported unlabeled dataset '{unlabeled_dataset}', skipping unlabeled data.")
                unlabeled_loader_class = None
            
            if unlabeled_loader_class is not None:
                print(f"\n[Semi-Supervised] Loading unlabeled data from {unlabeled_dataset}...")
                unlabeled_data = unlabeled_loader_class(
                    name="train_unlabeled",
                    data_path=config.TRAIN_UNLABELED.DATA.DATA_PATH,
                    config_data=config.TRAIN_UNLABELED.DATA,
                    device=config.DEVICE
                )
                data_loader_dict['train_unlabeled'] = DataLoader(
                ...
                print(f"[Semi-Supervised] Loaded {len(unlabeled_data)} unlabeled samples.\n")
            else:
                data_loader_dict['train_unlabeled'] = None
        else:
            data_loader_dict['train_unlabeled'] = None
        # ==================================================
        ...
        if (config.VALID.DATA.DATASET and config.VALID.DATA.DATA_PATH and not config.TEST.USE_LAST_EPOCH):
            extra_kwargs_valid = {}
            if use_multi_roi and config.VALID.DATA.DATASET == "COHFACE":
                extra_kwargs_valid['multi_roi_config'] = multi_roi_cfg
            ...
            valid_data = valid_loader(
                ...
                device=config.DEVICE,
                **extra_kwargs_valid)
            num_workers_valid = 0  # 单进程加载，避免F盘I/O并发问题
    ...
    if config.TOOLBOX_MODE == "train_and_test" or config.TOOLBOX_MODE == "only_test":
        ...
        if config.TEST.DATA.DATASET and config.TEST.DATA.DATA_PATH:
            extra_kwargs_test = {}
            if use_multi_roi and config.TEST.DATA.DATASET == "COHFACE":
                extra_kwargs_test['multi_roi_config'] = multi_roi_cfg
            ...
            test_data = test_loader(
                ...
                device=config.DEVICE,
                **extra_kwargs_test)
            num_workers_test = 0  # 单进程加载，避免F盘I/O并发问题
