...
_C.TRAIN.DATA.FS = 0
# 呼吸标签配置：在数据预处理时对PPG标签进行呼吸频带滤波
_C.TRAIN.DATA.USE_RESPIRATION_LABEL = False  # 是否使用呼吸频带滤波后的标签
_C.TRAIN.DATA.USE_REAL_RESPIRATION_LABEL = False  # 是否使用真实呼吸标签（如COHFACE）
_C.TRAIN.DATA.RESP_FREQ_MIN = 0.1  # 呼吸频带下限 (Hz), 6 breaths/min
_C.TRAIN.DATA.RESP_FREQ_MAX = 0.5  # 呼吸频带上限 (Hz), 30 breaths/min
# 呼吸标签提取方法: 'simple_bandpass', 'riiv', 'riav', 'rsa', 'edr', 'fusion'
# 推荐使用 'fusion' (融合RIIV+RIAV+RSA+EDR多种方法)
_C.TRAIN.DATA.RESP_EXTRACTION_METHOD = 'fusion'
...
# Phase 2: 光流特征增强配置
_C.TRAIN.DATA.PREPROCESS.USE_OPTICAL_FLOW = False  # 是否启用光流特征（4通道）
_C.TRAIN.DATA.PREPROCESS.USE_PHYSICS_MOTION = False  # 是否启用物理位移特征（dx,dy,mag -> 6通道）
_C.TRAIN.DATA.PREPROCESS.USE_CHEST_ROI = False     # 是否启用胸部ROI检测
...
_C.VALID.DATA.FS = 0
_C.VALID.DATA.USE_RESPIRATION_LABEL = False
_C.VALID.DATA.USE_REAL_RESPIRATION_LABEL = False  # 是否使用真实呼吸标签
_C.VALID.DATA.RESP_FREQ_MIN = 0.1
_C.VALID.DATA.RESP_FREQ_MAX = 0.5
_C.VALID.DATA.RESP_EXTRACTION_METHOD = 'fusion'
...
_C.VALID.DATA.PREPROCESS.USE_OPTICAL_FLOW = False
_C.VALID.DATA.PREPROCESS.USE_PHYSICS_MOTION = False  # 物理位移特征（dx,dy,mag -> 6通道）
_C.VALID.DATA.PREPROCESS.USE_CHEST_ROI = False
...
_C.TEST.DATA.FS = 0
_C.TEST.DATA.USE_RESPIRATION_LABEL = False
_C.TEST.DATA.USE_REAL_RESPIRATION_LABEL = False  # 是否使用真实呼吸标签
_C.TEST.DATA.RESP_FREQ_MIN = 0.1
_C.TEST.DATA.RESP_FREQ_MAX = 0.5
_C.TEST.DATA.RESP_EXTRACTION_METHOD = 'fusion'
...
_C.TEST.DATA.PREPROCESS.USE_OPTICAL_FLOW = False
_C.TEST.DATA.PREPROCESS.USE_PHYSICS_MOTION = False  # 物理位移特征（dx,dy,mag -> 6通道）
_C.TEST.DATA.PREPROCESS.USE_CHEST_ROI = False
...
_C.UNSUPERVISED.DATA.PREPROCESS.USE_OPTICAL_FLOW = False
_C.UNSUPERVISED.DATA.PREPROCESS.USE_PHYSICS_MOTION = False  # 物理位移特征（dx,dy,mag -> 6通道）
_C.UNSUPERVISED.DATA.PREPROCESS.USE_CHEST_ROI = False
...
# Frame number (通用帧数配置)
_C.MODEL.FRAME_NUM = 160

# STLAM (Spatial-Temporal Lateral Attention Mechanism) 配置
_C.MODEL.USE_STLAM = False  # 是否启用 STLAM 侧向注意力机制
_C.MODEL.MOTION_WEIGHT = 0.3  # 运动特征融合权重（0.1~0.5）
_C.MODEL.ATTENTION_SMOOTH_LAMBDA = 0.1  # 注意力时间平滑损失权重
_C.MODEL.ATTENTION_DIV_LAMBDA = 0.05  # 注意力多样性损失权重（避免坍塌到0.5）
...
# -----------------------------------------------------------------------------
# Respiration Detection settings (呼吸检测配置)
# 此配置节点用于启用呼吸率检测模式，不影响原有心率检测功能
# -----------------------------------------------------------------------------
_C.RESPIRATION = CN()
_C.RESPIRATION.ENABLE = False  # 是否启用呼吸检测模式
_C.RESPIRATION.USE_PPG_DERIVED_LABEL = True  # 是否从PPG标签推导呼吸参考信号
_C.RESPIRATION.FREQ_MIN = 0.1  # 呼吸频带下限 (Hz), 对应6 breaths/min
_C.RESPIRATION.FREQ_MAX = 0.5  # 呼吸频带上限 (Hz), 对应30 breaths/min
_C.RESPIRATION.FREQ_LOSS_WEIGHT = 0.3  # 频域损失权重（与Pearson损失的比例）

# -----------------------------------------------------------------------------
# Multi-ROI settings (多ROI配置 - 课题核心创新)
# 支持多区域ROI提取（面部x3 + 颈部x2 + 胸部x1）和跨ROI一致性损失
# -----------------------------------------------------------------------------
_C.MULTI_ROI = CN()
_C.MULTI_ROI.ENABLE = False  # 是否启用多ROI模式
_C.MULTI_ROI.NUM_ROIS = 6  # ROI区域数量
_C.MULTI_ROI.ROI_SIZE = [64, 64]  # ROI尺寸 [H, W]
_C.MULTI_ROI.USE_DUAL_BRANCH = False  # 是否使用双分支编码器
_C.MULTI_ROI.SHARE_ENCODER = True  # ROI间是否共享编码器权重
_C.MULTI_ROI.LAMBDA_CONSISTENCY = 0.5  # 跨ROI一致性损失权重
_C.MULTI_ROI.LAMBDA_AUX = 0.3  # ROI辅助损失权重（强制每个编码器学习）
_C.MULTI_ROI.USE_OPTICAL_FLOW = False  # 是否使用光流辅助

# --- Phase 2: 光流ROI增强配置 ---
# 以下配置需要在PREPROCESS节点中使用（TRAIN/VALID/TEST.DATA.PREPROCESS）
# 示例: TRAIN.DATA.PREPROCESS.USE_OPTICAL_FLOW = True
# 注意: 这些是文档说明，实际配置在各数据集的PREPROCESS下设置
# USE_OPTICAL_FLOW: False  # 是否使用光流特征（4通道输入）
# USE_CHEST_ROI: False     # 是否使用胸部ROI检测（MediaPipe）

# -----------------------------------------------------------------------------
# Phase 3: Semi-Supervised Learning settings (半监督学习配置)
# 基于Semi-rPPG论文实现的课程伪标签策略
# -----------------------------------------------------------------------------
_C.SEMI_SUPERVISED = CN()
_C.SEMI_SUPERVISED.ENABLE = False  # 是否启用半监督学习
_C.SEMI_SUPERVISED.LAMBDA_CONSISTENCY = 0.5  # 一致性损失权重
_C.SEMI_SUPERVISED.LAMBDA_PSEUDO = 0.5  # 伪标签损失权重
_C.SEMI_SUPERVISED.SNR_THRESHOLD_START = 10.0  # SNR阈值起始值（严格）
_C.SEMI_SUPERVISED.SNR_THRESHOLD_END = 5.0  # SNR阈值终止值（宽松）
_C.SEMI_SUPERVISED.CURRICULUM_EPOCHS = 20  # 课程学习持续的epoch数

# --- 无标注数据配置 ---
_C.SEMI_SUPERVISED.UNLABELED_DATA = CN()
_C.SEMI_SUPERVISED.UNLABELED_DATA.DATASET = ''  # 无标注数据集名称
_C.SEMI_SUPERVISED.UNLABELED_DATA.DATA_PATH = ''  # 无标注数据路径
_C.SEMI_SUPERVISED.UNLABELED_DATA.CACHED_PATH = ''  # 无标注数据缓存路径
...
def _update_config_from_file(config, cfg_file):
    ...
    # 使用UTF-8编码读取后合并，避免Windows GBK编码问题
    with open(cfg_file, 'r', encoding='utf-8') as f:
        yaml_content = f.read()
    from yacs.config import CfgNode as CN
...
def update_config(config, args):
    ...
    config.TRAIN.DATA.CACHED_PATH = os.path.normpath(os.path.join(config.TRAIN.DATA.CACHED_PATH, config.TRAIN.DATA.EXP_DATA_NAME))
    ...
    if not config.TEST.USE_LAST_EPOCH and config.VALID.DATA.DATASET is not None:
        ...
        config.VALID.DATA.CACHED_PATH = os.path.normpath(os.path.join(config.VALID.DATA.CACHED_PATH, config.VALID.DATA.EXP_DATA_NAME))
    ...
    config.TEST.DATA.CACHED_PATH = os.path.normpath(os.path.join(config.TEST.DATA.CACHED_PATH, config.TEST.DATA.EXP_DATA_NAME))
    ...
    config.UNSUPERVISED.DATA.CACHED_PATH = os.path.normpath(os.path.join(config.UNSUPERVISED.DATA.CACHED_PATH, config.UNSUPERVISED.DATA.EXP_DATA_NAME))
